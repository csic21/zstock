...
| **每日决策台** | 普通模式启动默认进入「今日」：按严重度汇总失效价触发、集中度超限、行情过期、未设失效价、价位提醒和到期复盘；候选按「符合观察条件 / 等待触发」分层，市场热力图降为「市场背景」二级入口 |
...
| **纪律仓位** | 决策卡按“计划本金 × 单笔亏损上限”反推最多买入股数，同时限制单票不超过 20%；自动考虑已有持仓，A 股按交易单位向下取整，可一键设好观察/失效/目标三道提醒或预填本地买入记录（不自动下单） |
| **透明决策过程** | 图表常驻显示当前结论与正在计算的环节；点开可逐步查看行情、技术规则、基本面、价位、仓位和最终动作，每步明确标记进行中/通过/注意/拦截及原因；选股后自动打开并展开 |
