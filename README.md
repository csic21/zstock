...
| **工作模式** | 标题栏 / `⌘⇧W`：整页服务监控台 + 迷你 spark；`p50` 对应现价，`drift` 对应涨跌幅，右侧 `cpu/mem/disk` 对应上证/沪深300/创业板真实点位；**按住 `` ` `` / Space 窥视**真身份（松手即藏）；`Map` 锁定约 6 秒后自动 Hide；`Tag` 给当前服务设私有昵称（写入 config）；窗口标题 Notes |
