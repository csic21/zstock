...
| **菜单栏行情** | macOS：设置里从自选固定最多 5 只，**多只同时**显示在系统菜单栏（紧凑拼接）；**未固定时显示 S logo**；下拉打开对应标的 / 唤起主窗口 |
...
| **AI 点评** | 一键生成中文分析（趋势/动量/量能/位置/风险 + **参考建仓/减仓元价位**）：默认本地规则即时出结果；可选接入 OpenAI 兼容 LLM（Responses / Chat 双协议），只上传指标快照，失败自动回退本地 |
