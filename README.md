...
| **纪律仓位** | 决策卡按“计划本金 × 单笔亏损上限”反推最多买入股数，同时限制单票不超过 20%；市场防守时自动缩小风险、观望日禁止新开仓；自动考虑已有持仓，A 股按交易单位向下取整，可一键设好观察/失效/目标三道提醒或预填本地买入记录（不自动下单） |
...
| **规则台账** | 今日页汇总日记 10 日结果：按策略、分数段、是否按计划、市场状态看胜率/期望/MFE/MAE；目标触及率低或先碰到失效时给出出场提示 |
| **轻量回测** | 策略 Tab：当前日 K 上跑「站上 MA20 持有 10 日」统计（次数/均收益/胜率/最差） |
| **AI 策略实验室** | 标题栏「策略实验室」：冻结数据集，生成 3–5 个白名单 DSL 策略（通用模板或雷达/寻宝扫描规则），在后台执行组合回测、walk-forward/成本压力/参数邻域检验，查看确定性排行榜与逐笔交易，并将过门槛版本加入幂等模拟观察；详见 [使用与研究约束](docs/strategy-lab.md) |
