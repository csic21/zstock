...
| **菜单栏行情** | macOS：设置里从自选固定最多 5 只，实时现价/涨跌幅显示在系统菜单栏；下拉切换展示 / 唤起主窗口 |
| **设置** | 标题栏 / `⌘,`：行情间隔、涨跌色、工作模式、菜单栏行情；写入本地 config |
| **🐭 寻宝鼠** | 多窗口历史低位扫描（1Y / 3Y / 全样本）；上行中继回撤降权；结果缓存；**候选池可选指数成分（沪深300 / 中证500 / 上证50 / 创业板指 / 科创50，动态拉取）或市值池，可加 PE/PB 横截面分位过滤** |
...
| **AI 点评** | 一键生成中文分析（趋势/动量/量能/位置/风险）：默认本地规则即时出结果；可选接入 OpenAI 兼容 LLM（Responses / Chat 双协议），只上传指标快照，失败自动回退本地 |
...
- **LLM 模式（可选）**：设置 → AI 分析 中开启并填写 API 地址 / 模型 / Key，
  支持 OpenAI **Responses** 与 **Chat Completions** 两种协议（如 DeepSeek、
  通义等兼容服务把协议切成 `Chat` 即可）。请求失败或未配置时自动回退本地点评。
- **来源标注**：每条点评上方标注来源（`本地规则` 或 `LLM · 模型名`），
  缓存命中时也会保留原来源；LLM 失败回退时同时显示失败原因。
- **隐私与成本**：只把本地算好的指标快照（JSON）发给模型，不上传原始 K 线与
  行情；Key 仅保存在本机 `config.json`。
