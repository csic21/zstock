...
| **🔍 现在找** | 统一入口：**长线**（寻宝低位 + 筛可买）/ **短线**（回踩·突破·超跌雷达）；标题栏「找长线/找短线」；缓存时效横幅（绿/黄/红）；⌘K 输入「长线」「短线」「市场」直达 |
...
| **AI 策略实验室** | 标题栏「策略实验室」：冻结数据集，生成 3–5 个白名单 DSL 策略（通用模板或雷达/寻宝扫描规则），在后台执行组合回测、walk-forward/成本压力/参数邻域检验，查看确定性排行榜与逐笔交易，并将过门槛版本加入幂等模拟观察；策略库按稳健分角逐冠军，每日观察会校正排名、淘汰弱势策略，并从冠军进化有界邻域变体；详见 [使用与研究约束](docs/strategy-lab.md) |
