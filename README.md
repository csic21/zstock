...
| **K 线 + 指标** | 前复权日 K；**内存序列缓存**（切股秒开，后台刷新）；MA5/10/20/60 + 成交量 / **MACD 副图** + **BOLL 通道**；周期 1M–MAX；十字线悬停 OHLC（含 MACD/BOLL/成本）；双击重置缩放 |
...
| **多面板** | 左自选/持仓/寻宝 / 中图表 / **底部分区分析台**（概览 · 策略 · AI · 持仓 · 寻宝 · 指标 Tab）；可拖拽调整宽高；**Dock + 左右/底部 Tab** 写入 config，重启恢复 |
| **命令面板** | `⌘K` / `⌘P` 搜索代码或名称 |
