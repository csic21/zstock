...
| **自选买入提醒** | 手动设置目标价，或一键采用本地技术参考建仓上沿；价格从上方跌入目标价时只提醒一次，重新上穿约 0.3% 自动重置；macOS 使用系统通知，提醒规则不依赖 AI |
| **持仓** | 买入/卖出/清仓流水；平均成本与浮动盈亏；**图表成本线**（虚线 + 标题栏成本/盈亏%）；可选现金约束；AI 结合成本+现价+技术面给出买卖观察建议；本地 `portfolio.json` |
...
| **每日决策台** | 普通模式启动默认进入「今日」：先看市场气候（进攻 / 精选 / 防守 / 观望），再按严重度处理失效价、集中度、行情过期和到期复盘；候选按气候门槛分成「符合观察条件 / 市场暂缓 / 等待触发」，市场热力图降为「市场背景」二级入口 |
| **🔍 现在找** | 统一入口：**长线**（寻宝低位 + 筛可买）/ **短线**（回踩·突破·超跌雷达）/ **连板**（首板·二连·二进一观察·炸板不追）；标题栏「找长线/找短线」；缓存时效横幅（绿/黄/红）；⌘K 输入「长线」「短线」「连板」「市场」直达 |
| **短线雷达** | 流动性 A 股池 → 三条本地策略（强势回踩 / 放量突破 / 超跌反弹）→ 可解释分数与观察带；策略过滤；一键入短线/长线池 |
| **连板梯队** | 今日涨幅榜 → 不复权 K 识别首板 / 二连 / 二进一观察 / 炸板；ST/退/次新/微成交额直接跳过，高板与炸板只看不做；每只带观察/失效/目标三价；防守日气候门冻结接力 |
| **自选分组** | 长线 / 短线 / 观察标签筛选；列表「标」循环切换；寻宝/雷达命中默认入对应池 |
| **价位提醒** | 买入观察 + **止盈** + **止损** 三腿；建议买/卖价一键填入；各腿触发一次后自动再武装 |
| **纪律仓位** | 决策卡按“计划本金 × 单笔亏损上限”反推最多买入股数，同时限制单票不超过 20%；市场防守时自动缩小风险、观望日禁止新开仓；自动考虑已有持仓，A 股按交易单位向下取整，可一键设好观察/失效/目标三道提醒或预填本地买入记录（不自动下单）。今日页会把策略库冠军应用到自选/持仓，给出每只股票买/卖多少股 |
| **透明决策过程** | 图表常驻显示当前结论与正在计算的环节；点开可逐步查看行情、市场气候、技术规则、基本面、价位、仓位和最终动作，每步明确标记进行中/通过/注意/拦截及原因；选股后自动打开并展开 |
| **板块下钻** | 市场分析页点击行业 → 成分股列表 → 点开图表并标短线池 |
| **规则台账** | 今日页汇总日记 10 日结果：按策略、分数段、是否按计划、市场状态看胜率/期望/MFE/MAE；目标触及率低或先碰到失效时给出出场提示；并展示策略库当前冠军与挑战者 |
| **轻量回测** | 策略 Tab：当前日 K 上跑「站上 MA20 持有 10 日」统计（次数/均收益/胜率/最差） |
| **AI 策略实验室** | 标题栏「策略实验室」：冻结数据集，生成 3–6 个白名单 DSL 策略（通用模板或雷达/寻宝/连板扫描规则），在后台执行组合回测、walk-forward/成本压力/参数邻域检验，查看确定性排行榜与逐笔交易，并将过门槛版本加入幂等模拟观察；策略库按稳健分角逐冠军，每日观察会校正排名、淘汰弱势策略，并从冠军进化有界邻域变体；详见 [使用与研究约束](docs/strategy-lab.md) |
| **静默预扫** | A 股休市且长线缓存空/过期时后台更新寻宝+筛可买；不切 Tab、保留旧榜；每 3h 再检查 |
| **决策日记** | 买/止盈/止损到价自动记一笔；概览可手写观察；本标的/全部筛选；本地 `journal.json` |
| **🐭 寻宝鼠（长线）** | ①扫低位 → ②**自动筛可买**（Top20 深评）→ 默认只看「可关注」与建仓价；完整寻宝榜折叠；可选 LLM 整榜摘要；上行中继/ST 降权 |
| **K 线 + 指标** | 前复权日 K；**内存序列缓存**（切股秒开 + 标题栏「刷新中」角标；冷启动 skeleton）；MA5/10/20/60 + 成交量 / **MACD 副图** + **BOLL 通道**；周期 1M–MAX；十字线悬停 OHLC（含 MACD/BOLL/成本）；双击重置缩放 |
...
| **多面板** | 左自选/持仓/寻宝 / 中图表 / **底部分区分析台**（概览 · 策略 · AI · 指标；持仓/寻宝列表在左侧，详情可从侧栏临时打开）；可拖拽调整宽高；**Dock + 左右/底部 Tab** 写入 config，重启恢复 |
| **命令面板** | `⌘K` / `⌘P` 搜索代码或名称；**↑↓ 选择 · Enter 确认**（无结果时 Enter 按输入代码添加） |
| **涨跌配色** | 标题栏切换：中国·红涨 / 美国·绿涨（自选、涨跌幅、K 线同步） |
| **工作模式** | 标题栏 / `⌘⇧W`：整页服务监控台 + 迷你 spark；`p50` 对应现价，`drift` 对应涨跌幅，右侧 `cpu/mem/disk` 对应上证/沪深300/创业板真实点位；**按住 `` ` `` / Space 窥视**真身份（松手即藏）；`Map` 锁定约 6 秒后自动 Hide；`Tag` 给当前服务设私有昵称（写入 config）；**Wide / Fit / Mini** 循环压缩布局与窗口（Mini ≈720×440，可拖左右分栏）；窗口标题 Notes |
...
| 快捷键 | 作用 |
|--------|------|
| `⌘K` / `Ctrl+K` | 命令面板（搜索 / 添加自选；↑↓ / Enter） |
| `⌘P` / `Ctrl+P` | 同上 |
| `↑` / `↓` · `Enter` | 命令面板打开时：选择 / 确认高亮项 |
| `⌘T` / `Ctrl+T` | 切换左侧「自选 / 现在找」 |
| `⌘⇧W` / `Ctrl+Shift+W` | 工作模式（中性配色与文案，标题变 Notes） |
| `` ` `` / `Space`（按住） | 工作模式：按住窥视真身份，松手恢复伪装 |
| `Map` / `Tag` | 工作模式标题栏：Map 锁定约 6s；Tag 设私有服务昵称 |
...
src/
  main.rs
  app/                # 主窗口（按职责拆分）
    mod.rs            # StockApp 状态、Render、run、快捷键
    types.rs          # ChartRange / LeftTab / DetailTab / AI 面板状态
    labels.rs         # 中文 / 工作模式文案
    market.rs         # 行情轮询、K 线/分时加载
    alerts.rs         # 自选买入提醒规则、持久化与通知触发
    chart_ctrl.rs     # 缩放平移、画线、paint 数据
    portfolio.rs      # 持仓流水与持仓 AI、config 防抖写入
    symbols.rs        # 自选、寻宝扫描、筛可买、命令面板
    prefs.rs          # 设置、工作模式、菜单栏、更新
    helpers.rs        # 纯展示/字符串辅助
    ui/               # 各区域 render_*
      chrome.rs       # 标题栏 + 全页设置
      left.rs         # 自选 / 持仓 / 寻宝列表
      chart.rs        # 图表区
...
其中 `config.json` 为自选与 UI 偏好，`treasure_cache.json` / `radar_cache.json` 为找票缓存，`journal.json` 为决策日记。
...
- 在底部「概览」的「自选买入提醒」中输入目标价并开启，或点击「用建议价」采用当前日 K 计算出的技术参考建仓上沿。
- 目标价在保存时固定，不会因为后续 K 线刷新而悄悄改变；提醒只在本地行情轮询中判断，不会自动下单。
- 价格从目标价上方跌入目标价区间时触发一次；重新站上目标价约 0.3% 后自动重新武装，也可手动重置或关闭。
- AI 适合补充“为什么观察这个价位”的文字解释，不适合决定触发条件。系统的目标价和提醒链路保持本地、确定性和可审计。
...
- [x] 「现在找」长线/短线双引擎 + 缓存时效 + 短线雷达
- [x] 自选分组、买/卖/止损提醒、板块下钻、轻量回测
- [x] 盘后/过期静默预扫长线榜；到价自动决策日记 + 手写备注
- [x] 规则历史表现跨标的汇总（今日规则台账 + 日记「按计划/未按计划」）
- [x] 雷达/寻宝扫描规则编进策略实验室；寻宝可关注受基本面质量门拦截
- [x] 今日市场气候门槛：弱市暂缓新开仓、缩放仓位、拦截追涨与抄底
- [ ] 港股纳入寻宝/短线池
