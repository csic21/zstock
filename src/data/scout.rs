...
pub fn llm_summary(cfg: &AiConfig, picks: &[ScoutPick]) -> anyhow::Result<String> {
    ...
    // 复用 ai 模块的 HTTP 层：构造最小 AiSnapshot 不合适，直接走内部协议
    llm_raw(cfg, user)
}

/// 直接对话（与 `ai::llm_commentary` 同协议）。
fn llm_raw(cfg: &AiConfig, user_prompt: String) -> anyhow::Result<String> {
...
不编造标的与数值，不给出具体仓位或收益承诺，强调观察价位非交易指令。";
    ...
    // 轻量解析：复用 ai 模块的公开路径较难（parse 为私有），这里内联最小解析。
    let v: serde_json::Value =
        serde_json::from_str(&text).context("解析 LLM 响应失败")?;
    if let Some(msg) = v
        .get("error")
