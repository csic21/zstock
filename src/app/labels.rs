...
impl L {
    ...
    pub(crate) fn detail_strategy(work: bool) -> &'static str {
        if work { "Signal" } else { "策略证据" }
    }

    pub(crate) fn detail_ai(work: bool) -> &'static str {
        if work { "AI" } else { "AI 解读" }
    }
    ...
    pub(crate) fn goto_ai(work: bool) -> &'static str {
        if work { "AI →" } else { "AI 解读 →" }
    }
