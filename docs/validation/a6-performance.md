...
5. Leave the same process running for at least 3,600 seconds after the first RSS sample.
6. Inspect `performance-report.json`. Require `validation_mode: true`, at least 20 navigation
   samples, `validation_chart_interaction_count: 120`, at least 61 RSS samples, non-null one-hour
   growth, and all four in-app budget helpers to pass. Insufficient navigation or RSS evidence
   produces no budget result rather than a false pass.
